...
def count_atoms(data, atom_list):
    atom_dict = {atom:i for i, atom in enumerate(atom_list)}
    atom_dict_rev = {i:atom for i, atom in enumerate(atom_list)}

    atom_count_dict = {}
    num_atom_mat = np.zeros((len(data), len(atom_list)))
    energy_vec = np.zeros((len(data),)) 

    for i, system in enumerate(data.keys()):
#         print(system)
        temp_atoms = data[system]["atoms"]
#         print(temp_atoms)
        atom_count_dict[system] = {}
        energy_vec[i] = data[system]["u0"]
        for atom in temp_atoms:
            num_atom_mat[i, atom_dict[atom]] += 1.0
#             print(atom, atom_count_dict[system])
            if atom in atom_count_dict[system]:
                atom_count_dict[system][atom] += 1
            else:
                atom_count_dict[system][atom] = 1
    print(num_atom_mat)
    print(energy_vec)
    ...
    atoms_count_list = []
    for i, system in enumerate(data.keys()):
        data[system]["corrected_energy"] = data[system]["u0"]
        num_atoms = 0
        for atom in atom_count_dict[system].keys():
            data[system]["corrected_energy"] -= atom_count_dict[system][atom] * correction_dict[atom]
            num_atoms += atom_count_dict[system][atom]
        corrected_energy_list.append(data[system]["corrected_energy"])
    ...
    result2 += "msepa: {}".format(np.mean(np.square(corrected_energy_per_atom_list)))
...
def linear_fit(data_list, atom_list, index_list):
    atom_dict = {atom:i for i, atom in enumerate(atom_list)}
    atom_dict_rev = {i:atom for i, atom in enumerate(atom_list)}
    ...
    num_atom_mat = np.zeros((len(index_list), len(atom_list)))
    energy_vec = np.zeros((len(index_list),)) 
    for i, system in enumerate(training_list):
        energy_vec[i] = system["u0_atom"]
...
def load_training_data(training_filename, test_filename, num_training, atom_list = ["H","C","N","O","F"]):
    ...
    try:
        ...
        with open('../pruned_qm9.json') as json_file: 
        ...
        index_list = random.sample(range(0, len(data_list)-1), num_training)
        #index_list = random.sample(range(0, len(data_list)-1), math.ceil(0.8 * len(data_list)))

        training_list, test_list = linear_fit(data_list, atom_list, index_list)
...
train_images, test_images = load_training_data(train_filename, test_filename, num_training)
